...
fn main() {
    ...
    //std::io::stdin se puede utilizar la libreria std:: aun sin importar al principio del codigo
        io::stdin()// Toma la entrada del usuario desde la consola
            .read_line(&mut guess)// Lee la linea de entrada y la guarda en la variable guess, el & permite que varias partes del corigo accedan a una pieza de datos sin sobreescribirla   
        ...
        }; // Convierte la cadena de texto en un numero entero sin signo de 32 bits, trim elimina los espacios en blanco y parse convierte la cadena en un numero, si hay un error imprime el mensaje de error
        ...
        match guess.cmp(&secret_number) { // Compara la variable guess con la variable secret_number
            ...
            Ordering::Equal => {
                println!("You win!"); // Si guess es igual que secret_number, imprime "You win!"
                break; // Sale del loop
            }
        }
    }
